...
The vault is the Obsidian vault in `~/Documents` — find it by its `.obsidian` marker directory:

```bash
# vault dir name may contain spaces — quote everything downstream
VAULT=$(dirname "$(find ~/Documents -maxdepth 2 -type d -name .obsidian -print -quit)")
